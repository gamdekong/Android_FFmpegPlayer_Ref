...
Java_company_co_kr_videoplayer_AudioPlayer_nativePlayAudio(JNIEnv *env, jobject instance,
                                                           jstring fileName_) {

    LOGV("nativePlayAudio()");
    const char *fileName = (char*)fileName_;
...
void playMp3(const char *filename) {
    ...
    if ((avformat_find_stream_info(formatContext, NULL)) < 0) {
        LOGE("avformat_find_stream_info() error %d");
        return;
    }
    ...
    while (av_read_frame(formatContext, packet) == 0) {
        if (packet->stream_index == audioStream && avcodec_send_packet(c, packet) == 0) {
            while (avcodec_receive_frame(c, frame) == 0) {
                ...
                if (stream == NULL) {
                    stream = android_OpenAudioDevice(frame->sample_rate, 1, 1,
                                                     frame->linesize[0] / sizeof(short));
                }
